...
namespace Seal.Forms
{
    public partial class SQLEditorForm : Form
    {
        ...
        public void checkSQL()
        {
            ...
            {
                var m = (ReportModel)Instance;
                if (m.Source.Connection.DatabaseType == DatabaseType.Redis) RedisServer = true;
                connectionString = m.Source.Connection.ConnectionString;
                redisKeyName = m.Elements[0].DisplayNameEl;
            }
            ...
            if (RedisServer)
            {
                ...
                Boolean existsKey = true;//redis.Haskey(redisKeyName) == 0;
                if (existsKey)
                {
                    toolStripStatusLabel.Text = "SQL checked successfullyhahah";
