            ...
            else if (!string.IsNullOrEmpty(_sql) && !Report.Cancel)
            {
                ...
                try
                {
                    ...
                    if (ResultTable == null)
                    {
                        ...
                        if (Connection.DatabaseType == DatabaseType.Redis)
                        {
                            ...
                            foreach (var col in this.Elements)
                                ResultTable.Columns.Add(col.RawDisplayName);

                            for(int index = 0; index < getRedisData.Count; index++)
                            {
                                row = ResultTable.NewRow();
                                foreach(var col in this.Elements)
                                    row[col.RawDisplayName] = getRedisData[index][col.RawDisplayName];
