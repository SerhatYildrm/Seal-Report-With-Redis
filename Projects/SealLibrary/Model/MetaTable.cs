...
namespace Seal.Model
{
    public class MetaTable : RootComponent, ReportExecutionLog
    {
        ...
        public void CheckTable(MetaColumn column)
        {
            ...
            /*Writer:Serhat YIldırım*/

            if (this._source.Connection.DatabaseType == DatabaseType.Redis)
            {
                Redis redis = new Redis();
                redis.Connect(this._source.Connection.ConnectionString);
                string key = Source.MetaData.Tables[0].Sql;

                if (1 == redis.HasKey(key))
                {
                    _information = key + " Ulaşıldı.";
                }
        ...
        public string ShowValues(MetaColumn column)
        {
            ...
            try
            {
                ...
                if (_source.Connection.DatabaseType == DatabaseType.Redis)
                {

                    string sql = column.DisplayName;

                    Redis redis = new Redis();
                    redis.Connect(_source.Connection.ConnectionString);
                    var datas = redis.GetData(_source.Connection.Source.MetaData.Tables[0].Sql);
